"""
dispatcher.policy_store
───────────────────────
Ported from archiver.policy_store. Same Repository pattern, same atomic
write semantics, same hierarchical (user → platform → global) lookup.

Why we PORT rather than IMPORT:
  Architectural rule from the implementation guide: dispatcher, archiver,
  and recorder communicate via SQLite ONLY. Sharing Python modules across
  packages creates an install-order dependency we don't want. Copying
  ~280 LoC of stable code is the right tradeoff.

If you fix a bug here, mirror it into archiver/policy_store.py. The two
copies WILL drift; that's accepted. Drift only matters when both copies
share a bug, in which case grep both files.

Storage layout (this dispatcher's file):
  ~/.config/dispatcher/config.toml

Overridable via $DISPATCHER_CONFIG for tests.
"""
...
_HEADER = """\
# dispatcher config — machine-managed but human-readable.
...
def default_config_path() -> Path:
    """Canonical config location. Override with $DISPATCHER_CONFIG."""
    override = os.environ.get("DISPATCHER_CONFIG")
    if override:
        return Path(override).expanduser()
    return Path.home() / ".config" / "dispatcher" / "config.toml"
